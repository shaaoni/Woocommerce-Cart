...
function custom_image_sizes() {
    // Add custom image size with width 250px and height 250px
    add_image_size('custom-size',393,440, true); // Change 'custom-size' to your desired size name
}

// Function to add custom field for marking products as best sellers
function add_best_seller_custom_field() {
    global $post;

    // Add a custom meta box
    add_meta_box(
        'best_seller_meta_box', // Meta box ID
        'Best Seller', // Title of the meta box
        'display_best_seller_custom_field', // Callback function to display the custom field
        'product', // Post type where the meta box should be displayed
        'side', // Context (e.g., 'normal', 'side', 'advanced')
        'default' // Priority (e.g., 'high', 'core', 'default', 'low')
    );
}
add_action( 'add_meta_boxes', 'add_best_seller_custom_field' );

// Callback function to display the custom field
function display_best_seller_custom_field( $post ) {
    $is_best_seller = get_post_meta( $post->ID, '_is_best_seller', true );
    ?>
    <label for="is_best_seller">
        <input type="checkbox" name="is_best_seller" id="is_best_seller" <?php checked( $is_best_seller, 'yes' ); ?>>
        <?php _e( 'Mark as Best Seller', 'textdomain' ); ?>
    </label>
    <?php
}

// Save custom field value when the product is saved
function save_best_seller_custom_field( $post_id ) {
    if ( isset( $_POST['is_best_seller'] ) ) {
        update_post_meta( $post_id, '_is_best_seller', 'yes' );
    } else {
        delete_post_meta( $post_id, '_is_best_seller' );
    }
}
add_action( 'save_post_product', 'save_best_seller_custom_field' );
...
function custom_mini_cart() { 
    ...
    echo '<a href="'.home_url('cart').'">';
        echo '<h6 class="font-12 font-700"> CART';
        echo '<span class="font-400" id="cart_quantity">';
            echo $cart_count;
        echo '</span>';
    echo '</h6></a>';
    // echo '<div style="float:right; width:auto;">';
    //     echo '<span class="display:flex; ">';
    //          echo '<span >';
...
function custom_add_to_cart_callback() {
    if (isset($_POST['product_id'])) {
        ...
        if (false !== WC()->cart->add_to_cart($product_id)) {
            // Get updated cart quantity
            $cart_quantity = WC()->cart->get_cart_contents_count();
			//WC_AJAX::get_refreshed_fragments();

            // Return fragments and cart quantity
            // $response = array(
            //     'fragments'     => ,
            //     'cart_quantity' => $cart_quantity,
			// 	'mini_cart_html'  => custom_mini_cart(),
            // );

         echo $cart_quantity;//json_encode($response);
        }
...
function update_total_price_callback() {
    ...
    // Echo the updated total price
    echo $total_price;
	exit();
    //wp_die();
}

function custom_wishlist_setup() {
    // add_theme_support( 'woocommerce', array(
    //     'template_path' => 'woocommerce',
    //     'template_override' => 'wishlist/'
    // ) );
	add_theme_support( 'wp-content/themes/WooCart/wishlist' );
	// add_theme_support('wishlist');
}
add_action( 'after_setup_theme', 'custom_wishlist_setup' );
